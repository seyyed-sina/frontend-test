...
const POST_TO_SHOW = 5; // Number of posts to fetch

// function showSkeletons(count: number = POST_TO_SHOW) {
...
//     skeleton.className =
//       "animate-pulse bg-white p-6 rounded-lg shadow space-y-4";
//     skeleton.innerHTML = `
...
async function main() {
  try {
    // showSkeletons();
    const topPosts = await fetchTopPosts(POST_TO_SHOW);
